...
![Object Diagram](https://github.com/Areenor/SD/blob/Assignment-3/docs/Object_Diagram_Ass3.png)
...
## Implementation									
Author(s): Zakhar Zhornovyi, Richard Eric van Leeuwen(Video)

We followed a dynamic process to move from our UML diagrams to the implementation of our code, in other words we created part of the diagrams, then translated them to code, observed if there were things in the implementation which we were not satisfied with or which required some modifications, if so we adjusted our diagrams accordingly, and then moved on to creating the next part of our diagrams. This method was especially useful when we were selecting the types of the attributes of the classes used in our diagram as we went back and forth between our code and our diagram, in order to find what would work best. Creating of the associations was mostly done in the diagrams only, and later copied to the code, while creations of some classes(__InitiationServise__ and __Combat__ processes) was firstly completely implemented and only after that added to diagrams. As for the __InitiationServise__ class, we firstly had to figure out how it would work codewise before we could proceed with diagram. For the __Combat__ class, the system we pictures was quite complex at first to put it in diagram, so we decided to firstly code it through to find out our boundaries. 

Our program utilizes a set of __Location__ instances which function as the “playing field” of the game. The locations contains instances of both __NPC__ subclass and subclasses of __Item__ superclass(such as __Equipment__ or __Consumable__), as same as a main character which is of a subclass __Player__. At all times there is a copy of a __Location__ instance assigned to a static variable called _ currentLocation_ which is part of the __Player__ class, this variable reflects the location in which the player’s character is currently present. The player’s is an instance of subclass __Player__ and is named _MainCharacter_. It is worth noting that both __Player__ and __NPC__ are the subclasses of abstract superclass __Character__, as they share set of variables such as _name_, _strength_ etc. 

To handle the execution of commands, the __Controller__ class was implemented. It contains functions _ExecuteCommand_ and _ExecuteCombatCommand_, for either of which __Main__ class calls until the game is finished. __Controller__ class asks for input of the command, and after certain checking passes it to a __Player__ class, where the requested action is executed.

The player can orientate himself and figure which commands to use in order to make progress based on _currentLocation.description_ which is printed to the terminal upon entering the location or using the examine command (without specifying a target). Moving between locations is done by reassigning _currentLocation_ in the _Move_ function in __Player__ subclass with using a hashmap of directions and _adjacentLocations_. Implementing movement between locations this way allows for a way of navigating between locations without having to construct some kind of map in the program, something which would be extra complicated because vertical movement between locations is allowed. Additionally, storing a copy of the __Location__ instance in which the main character is present in _currentLocation_ allows for access to this instance from anywhere in the code, giving more flexibility during the programming process. 

We decided to handle combat by creating a separate __Combat__ service class. Our implementation of combat system was inspired by oldschool games with turn-based combat systems such as Final Fantasy and tabletop games such as Dungeon&Dragons (as text video games was originally inspired by it). Combat flow is as follows: there is an array list of all participants(called _combatOrder_), including the main character. Until only _Main Character_ is left or until _Main Character_ is dead, the _Run_ function of __Combat__ is executed by __Main__ and each participant takes his turn. Initiation of the combat (with the help of _Init_ function) is performed either when main character change _currentLocation_ to the location with _hostile_ NPCs (with special check of  _GetEnemies_ function from __Location__ class) or if player commanded to _Attack_ fightable _NPC_.

Most instances of classes used in the program are initiated during startup, this includes all locations, the NPCs and items contained by these locations, and the __Character__ instance for the main character. The initiation of instances of the __Location__, __NPC__ and __Item__ classes is done using JSON files. A JSON file is read into a special config class, __LocationConfig__, __NpcConfig__ and __ItemConfig__,  which is then used to create the instance of the corresponding class. On startup, a special __InitiationService__ class will iterate over all JSON files containing configurations for the __Location__ instances and use them to create the locations. The names of the items and NPCs given in a location JSON file correspond to the JSON files belonging to those items and NPCs, allowing __InitiationService__ to create instances of these items and NPCs and place them in the _items_ and _characters_ hashmaps of the __Location__ instance. The use of JSON files in this way makes it very easy to create a custom story for the game.
